...
EntityESP.update = LPH_NO_VIRTUALIZE(function(self, tags)
	...
	-- Update poison bar if it exists (for MobESP)
	local pbar = self.pbar
	local poisonBar = pbar and self.gb(pbar)

	if pbar and poisonBar and self.lastPoisonValue ~= nil then
		-- Get poison value - use stored value if available
		local poisonValue = self.lastPoisonValue
		
		-- Assume max poison is 100 for percentage calculation
		local maxPoison = 100
		local percent = math.clamp(poisonValue / maxPoison, 0.0, 1.0)
		
		local fullColor = Configuration.idOptionValue(identifier, "PoisonFullColor") or Color3.new(0.5, 1, 0)
		local emptyColor = Configuration.idOptionValue(identifier, "PoisonEmptyColor") or Color3.new(0.2, 0.5, 0)

		-- Update sizing.
		self.mbs(pbar, true, percent)

		-- Update color.
		poisonBar.BackgroundColor3 = emptyColor:Lerp(fullColor, percent)
	end
