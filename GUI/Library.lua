...
return LPH_NO_VIRTUALIZE(function()
	...
	do
		...
		-- Event Timer GUI
		local EventTimerOuter = Library:Create("Frame", {
			BorderColor3 = Color3.new(0, 0, 0),
		...
		local EventTimerInner = Library:Create("Frame", {
			BackgroundColor3 = Library.MainColor,
			BorderColor3 = Library.OutlineColor,
			BorderMode = Enum.BorderMode.Inset,
			Size = UDim2.new(1, 0, 1, 0),
			ZIndex = 201,
			Parent = EventTimerOuter,
		})

		Library:AddToRegistry(EventTimerInner, {
			BackgroundColor3 = "MainColor",
			BorderColor3 = "OutlineColor",
		}, true)

		local EventColorFrame = Library:Create("Frame", {
			BackgroundColor3 = Library.AccentColor,
			BorderSizePixel = 0,
			Size = UDim2.new(1, 0, 0, 2),
			ZIndex = 204,
			Parent = EventTimerInner,
		})

		Library:AddToRegistry(EventColorFrame, {
			BackgroundColor3 = "AccentColor",
		}, true)

		local InnerFrameEvent = Library:Create("Frame", {
			BackgroundColor3 = Color3.new(1, 1, 1),
			BorderSizePixel = 0,
			Position = UDim2.new(0, 1, 0, 1),
			Size = UDim2.new(1, -2, 1, -2),
		...
		local GradientEvent = Library:Create("UIGradient", {
			Color = ColorSequence.new({
				ColorSequenceKeypoint.new(0, Library:GetDarkerColor(Library.MainColor)),
				ColorSequenceKeypoint.new(1, Library.MainColor),
			}),
			Rotation = -90,
			Parent = InnerFrameEvent,
		})

		Library:AddToRegistry(GradientEvent, {
			Color = function()
				return ColorSequence.new({
					ColorSequenceKeypoint.new(0, Library:GetDarkerColor(Library.MainColor)),
					ColorSequenceKeypoint.new(1, Library.MainColor),
		...
		local EventTimerLabel = Library:CreateLabel({
			Position = UDim2.new(0, 5, 0, 1),
		...
		Library:AddToRegistry(EventTimerLabel, {
			TextColor3 = "AccentColor",
		}, true)

		local EventCountdownLabel = Library:CreateLabel({
		...
		Library:AddToRegistry(EventCountdownLabel, {
			TextColor3 = "AccentColor",
		}, true)

		Library.EventTimerFrame = EventTimerOuter
		Library.EventTimerLabel = EventTimerLabel
		Library.EventTimerCountdownLabel = EventCountdownLabel
		
		-- Store in global so RenderStepped loop can access them
		_G.EventTimerElements = {
			frame = EventTimerOuter,
			label = EventTimerLabel,
			countdown = EventCountdownLabel
		}
		
		Library:MakeDraggable(Library.EventTimerFrame)
