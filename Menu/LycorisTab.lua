...
function LycorisTab.initUISettingsSection(groupbox)
    ...
    local EventTimer = require("Utility/EventTimer")
    local timezones = {"NA Central (CST)", "NA East (EST)", "NA Mountain (MST)", "NA Pacific (PST)", "AU (AEDT)",
                       "EU (GMT)", "JP (JST)"}

    local eventTimerLabel = groupbox:AddLabel("Event Timer")

    eventTimerLabel:AddKeyPicker("EventTimer", {
        Default = "N/A",
        Mode = "Off",
        NoUI = true,
        Text = "Event Timer",
        Callback = function(Value)
            if Library.EventTimerFrame then
                Library.EventTimerFrame.Visible = Value
                if Value then
                    -- refresh immediately when toggled via keybind
                    local eventName, secondsUntil, eventTime = EventTimer.getNextEvent()
                    local countdown = EventTimer.formatCountdown(secondsUntil)
                    Library.EventTimerLabel.Text = eventName .. " @ " .. eventTime
                    Library.EventTimerCountdownLabel.Text = "Starts in: " .. countdown
                end
            end
        end
    })

    groupbox:AddToggle("ShowEventTimer", {
        Text = "Show Event Timer",
        Default = false,
        Callback = function(Value)
            if Library.EventTimerFrame then
                Library.EventTimerFrame.Visible = Value
                if Value then
                    -- immediately update labels when shown
                    local eventName, secondsUntil, eventTime = EventTimer.getNextEvent()
                    local countdown = EventTimer.formatCountdown(secondsUntil)
                    Library.EventTimerLabel.Text = eventName .. " @ " .. eventTime
                    Library.EventTimerCountdownLabel.Text = "Starts in: " .. countdown
    ...
    groupbox:AddDropdown("EventTimerTimezone", {
        Text = "Event Timer Timezone",
        Values = timezones,
        Default = 2,
        Multi = false,
        Callback = function(Value)
            EventTimer.setTimezone(timezones[Value])
        end
    })
end
