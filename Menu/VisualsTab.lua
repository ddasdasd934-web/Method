...
function VisualsTab.addEntityESP(identifier, depbox)
	...
	if identifier == "Mob" then
		local pbToggle = depbox:AddToggle(Configuration.identify(identifier, "PoisonBar"), {
			Text = "Show Poison Bar",
			Default = false,
		})

		pbToggle:AddColorPicker(Configuration.identify(identifier, "PoisonFullColor"), {
			Default = Color3.new(0.5, 1, 0),
		})

		pbToggle:AddColorPicker(Configuration.identify(identifier, "PoisonEmptyColor"), {
			Default = Color3.new(0.2, 0.5, 0),
		})
	end
	...
	if identifier == "Mob" then
		depbox:AddToggle("ShowMobBloodPoison", {
			Text = "Show Blood Poison",
			Default = false,
		})
	end
